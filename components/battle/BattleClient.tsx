...
function clearBattleSession(roomId: string) {
  try {
    sessionStorage.removeItem(`battle_reps_${roomId}`);
    sessionStorage.removeItem(`battle_screen_${roomId}`);
  } catch {}
}
...
export default function BattleClient({ roomId }: Props) {
  ...
  async function syncScore(reps: number) {
    ...
    saveSession(`battle_reps_${roomId}`, reps.toString());
    const field = isHostRef.current ? "host_score" : "guest_score";
            ...
            <div style={{
              ...
              {[
                ...
                <div key={i} style={{ textAlign:"center" }}>
                  ...
                  <div style={{ fontSize:9, opacity:0.35, letterSpacing:1, marginTop:2 }}>
                    {s.label}
                  </div>
                </div>
              ))}

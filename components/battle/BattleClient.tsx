...
// Score sync interval — push score to Supabase every N reps
...
function checkPosition(lms:Landmark[]):{ok:boolean;checks:CheckResult[];angles:Angles}{
  ...
  const angles:Angles={elbow:elbowAngle,body:bodyAngle,knee:kneeAngle,hipOffset:hipOffsetPct,verticalSpan};
  ...
  checks.push({label:"Body flat",pass:isHoriz,value:`span ${verticalSpan}%`,hint:isHoriz?"✓":"Lie flat"});
  ...
  checks.push({label:"Legs straight",pass:kneesOk,value:`knee ${kneeAngle}°`,hint:kneesOk?"✓":"Lift knees"});
  const bodyOk=bodyAngle>=BODY_ANGLE_MIN;
  checks.push({label:"Body straight",pass:bodyOk,value:`body ${bodyAngle}°`,hint:bodyOk?"✓":"Tighten core"});
...
function speak(text:string,rate=1.0){
  ...
  const preferred=voices.find(v=>v.name.toLowerCase().includes("male")||v.name.toLowerCase().includes("daniel")||v.name.toLowerCase().includes("alex"));
...
export default function BattleClient({ roomId }:Props) {
  ...
  const lastSyncedRef   = useRef(0); // last rep count synced to Supabase
  ...
  // ── Load room + determine role ──────────────────────────────────
  useEffect(()=>{
    async function loadRoom(){
      const{data}=await supabase.from("rooms").select("*").eq("id",roomId).single();
      if(data){
        setRoom(data as Room);
        isHostRef.current = (data.host_id===playerId);
        // Set initial opp score
        const oppScoreVal = isHostRef.current ? data.guest_score : data.host_score;
  ...
  useEffect(()=>{
    const channel=supabase
      ...
      .on("postgres_changes",{
        ...
        setRoom(updated);
        const oppScoreVal=isHostRef.current?updated.guest_score:updated.host_score;
        setOppScore(oppScoreVal);
        // If battle finished by other player
  ...
  async function startCamera(){
    const stream=await navigator.mediaDevices.getUserMedia({
      video:{width:{ideal:640},height:{ideal:480},frameRate:{ideal:30,max:30},facingMode:"user"},audio:false,
    });
  ...
  function resetRefs(){
    phaseRef.current="no_plank"; posOkRef.current=false; posHoldRef.current=0;
    repCountRef.current=0; lastRepRef.current=0; screenRef.current="setup";
    battleStartRef.current=0; lastSyncedRef.current=0;
  ...
  function beginCountdown(){
    screenRef.current="countdown"; setScreen("countdown");
  ...
  function beginBattle(){
    screenRef.current="battle"; battleStartRef.current=performance.now();
    phaseRef.current="no_plank"; repCountRef.current=0;
    setScreen("battle"); setRepCount(0); setBattleTime(BATTLE_DURATION_SEC);
  }

  async function endBattle(){
    screenRef.current="results"; setScreen("results");
    window.speechSynthesis?.cancel();
    // Sync final score
    await syncScore(repCountRef.current);
    // Host marks battle finished and saves match
    ...
    if(r>=20) speak(`BEAST MODE! ${r} reps!`,1.1);
    else if(r>=10) speak(`${r} reps! Not bad!`,1.0);
    else speak(`${r} reps. Run it back!`,0.95);
  }
  ...
  function processPose(lms:Landmark[]){
    ...
    const wasOk=posOkRef.current, nowOk=result.ok;
    posOkRef.current=nowOk; setPositionOk(nowOk);
        ...
        case "ascending":
          if(eAngle>=TOP_ELBOW){
            if(now-lastRepRef.current>=MIN_REP_MS){
              ...
              // Sync every N reps
              if(repCountRef.current%SYNC_EVERY_N_REPS===0){
                syncScore(repCountRef.current);
              }
            }
            phaseRef.current="top";setPhase("top");
          } else if(eAngle<=BOTTOM_ELBOW){phaseRef.current="bottom";setPhase("bottom");}
          break;
      }
    }
  }
  ...
  function drawPose(lms:Landmark[],currentPhase:PushupPhase,isOk:boolean){
    ...
    const phaseColors:Record<PushupPhase,string>={
      no_plank:"#ff2244",top:"#00ff88",descending:"#ffcc00",bottom:"#ff6644",ascending:"#44aaff",
    };
  ...
  // Determine win/loss for results
  const myScore  = repCount;
  const didWin   = oppScore !== null ? myScore > oppScore : null;
    ...
    <div style={{
      ...
      {screen==="setup"&&(
        <div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",justifyContent:"space-between",padding:20}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              <div style={{fontSize:22,fontWeight:900,letterSpacing:-1}}>LOCKED'N<span style={{color:"#00ff88"}}>.</span></div>
          ...
          <div style={{
            ...
            <div style={{fontSize:11,opacity:0.4,letterSpacing:2,marginBottom:10}}>POSITION CHECK</div>
            {checks.map((c,i)=>(
              <div key={i} style={{
                ...
                <span style={{fontSize:13,fontWeight:700,flex:1,color:c.pass?"#00ff88":"#ff4466"}}>{c.label}</span>
            ...
            {positionOk&&(
              <div style={{marginTop:14}}>
                ...
                <div style={{fontSize:11,opacity:0.4,marginTop:6,textAlign:"center"}}>Starting automatically...</div>
              </div>
            )}
          </div>
          <div style={{alignSelf:"flex-end",opacity:0.3,fontSize:11}}>{fps}fps · {landmarks}/33</div>
        </div>
      )}
      ...
      {screen==="countdown"&&(
        <div style={{
          ...
          <div style={{fontSize:13,opacity:0.4}}>{countdown>0?`${BATTLE_DURATION_SEC}s pushup battle`:""}</div>
        </div>
      )}
      ...
      {screen==="battle"&&(
        <div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",justifyContent:"space-between"}}>
          <div style={{padding:"20px 20px 0",display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
            {/* My score */}
            <div>
              <div style={{fontSize:100,fontWeight:900,lineHeight:0.9,letterSpacing:-5,color:"#00ff88",textShadow:"0 0 40px rgba(0,255,136,0.4)"}}>
                {repCount}
              </div>
          ...
          <div style={{display:"flex",justifyContent:"center"}}>
            <div style={{
              ...
              border:`1px solid ${phaseColor[phase]}55`,color:phaseColor[phase],letterSpacing:1,
            }}>
              {phaseLabel[phase]}
            </div>
          </div>

          <div style={{padding:"0 20px 28px",display:"flex",justifyContent:"space-between",alignItems:"flex-end"}}>
            <div style={{
              ...
              <div style={{width:7,height:7,borderRadius:"50%",background:positionOk?"#00ff88":"#ff2244",boxShadow:positionOk?"0 0 6px #00ff88":"0 0 6px #ff2244"}}/>
      ...
      {screen==="results"&&(
        <div style={{
          ...
          <div style={{
            ...
            <div style={{
              background: didWin?"linear-gradient(90deg,#00ff8822,#00ccff22)":"linear-gradient(90deg,#ff446622,#ff664422)",
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr auto 1fr",alignItems:"center",padding:"28px 20px"}}>
              <div style={{textAlign:"center"}}>
                ...
                <div style={{fontSize:72,fontWeight:900,letterSpacing:-4,color:"#00ff88",lineHeight:1}}>{myScore}</div>
                <div style={{fontSize:11,opacity:0.4,marginTop:4}}>REPS</div>
              </div>
              <div style={{padding:"8px 16px",fontSize:14,fontWeight:900,opacity:0.25,letterSpacing:2}}>VS</div>
              <div style={{textAlign:"center"}}>
                ...
                <div style={{
                  ...
                  color:oppScore!==null?"rgba(255,255,255,0.8)":"rgba(255,255,255,0.2)",
                }}>
            ...
            <div style={{
              ...
              {[
                {label:"MY REPS",value:myScore},
                {label:"DURATION",value:`${BATTLE_DURATION_SEC}s`},
                {label:"RPM",value:Math.round((myScore/BATTLE_DURATION_SEC)*60)},
              ].map((s,i)=>(
          ...
          <div style={{marginTop:24,fontSize:13,opacity:0.4,textAlign:"center",maxWidth:280}}>
            {myScore>=20?"Absolute beast. Challenge someone now.":
             myScore>=10?"Solid. You've got more in the tank.":
          ...
          <div style={{display:"flex",gap:12,marginTop:28,width:"100%",maxWidth:380}}>
            <button onClick={()=>router.push(`/room/${roomId}`)} style={{
              ...
              background:"#00ff88",color:"#000",fontSize:15,fontWeight:900,cursor:"pointer",
            }}>
              REMATCH 🔥
            </button>
            <button onClick={()=>{
              if(navigator.share){
                navigator.share({title:"Locked'N",text:`Just hit ${myScore} pushups in ${BATTLE_DURATION_SEC}s on Locked'N 💪`,url:window.location.href});
              }
            }} style={{
              padding:"16px 18px",borderRadius:14,
              border:"1px solid rgba(255,255,255,0.15)",
              background:"transparent",color:"white",fontSize:15,fontWeight:700,cursor:"pointer",
            }}>SHARE</button>
          </div>
        </div>
      )}
